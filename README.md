...
일반적인 경로 탐색은 물리적 거리만 고려하지만 이 프로젝트는 **이동의 불확실성**을 반영합니다.

- 목적지까지 거리가 3인 위치 A: 매번 90% 확률로 목적지 방향으로 이동
- 목적지까지 거리가 2인 위치 B: 매번 30% 확률로 목적지 방향으로 이동

단순 거리로는 B가 더 가깝지만 확률을 고려하면 A가 더 효율적인 경로일 수 있습니다.
이 프로젝트는 마르코프 리워드 프로세스를 통해 이러한 확률적 가치를 계산하고
실제 시뮬레이션을 통해 불확실한 환경에서 목적지에 도달하는 과정을 보여줍니다.

### GUI 실행 화면

지도의 색깔은 목적지까지 가는 데 드는 비용을 의미합니다.
초록색일수록 비용이 적게 드는 효율적인 경로이고 빨간색일수록 비용이 많이 드는 비효율적인 위치입니다.
단순히 거리만 계산하는 것이 아니라 이동 확률을 반영하기 때문에 같은 거리라도 확률에 따라 비용이 다르게 계산됩니다.
캐릭터는 지도에 설정된 확률에 따라 움직이며 시뮬레이션을 진행합니다.

![app-gui](./assets/app-gui-execution.gif)

### CLI 실행 화면

GUI와 동일한 기능을 텍스트로 표현합니다.
지도의 각 위치에 대한 비용은 숫자로 표현되며 숫자가 낮을수록 목적지에 가깝고 유리한 위치라는 뜻이며 최종 목적지는 비용이 `Cost.DESTINATION`으로 표시됩니다.

<img src="./assets/app-cli-execution.png" height="700px">
...
- **참고**: 초기 개발 단계에서 CLI 기반으로 테스트를 진행해 core에서 출력 없이 테스트되어야 할 몇가지 주요 테스트가 `app-cli` 모듈에 포함되어 있습니다...
...
이 프로젝트의 핵심은 불확실한 확률 환경에서 목적지까지 도달하기 위한 각 위치의 잠재적 비용을 계산하는 것(마르코프 리워드 프로세스)입니다.
이를 위해 벨만 방정식을 해결함으로써 해(각 위치의 잠재적 비용)를 구합니다.
...
해당 과정은 `CostMapController`가 제어합니다.
...
- `CostMapController`: 최적의 비용 지도를 찾기 위해 벨만 방적식을 풀어냅니다
...
초기에는 마르코프 결정 과정을 구현한 뒤 Value Iteration과 Policy Iteration의 성능을 JMH(Java Microbenchmark Harness) 기반으로 비교하는 프로젝트를 계획했습니다.
두 알고리즘의 계산 복잡도를 정량적으로 비교하는 작업이 충분히 흥미롭고 의미 있는 실험이라고 판단했기 때문입니다.

그러나 구현 범위를 조정하는 과정에서 프로젝트의 초점을 마르코프 리워드 프로세스로 전환하였고 JMH 기반의 알고리즘 성능 비교는 진행하지 않게 되었습니다.

아래는 프로젝트와 별도로 JMH 학습 내용을 바탕으로 수행한 3-2 알고리즘 수업 과제 자료입니다.

1. [과제 보고서](./assets/Assignment2(알고리즘)_2021202048장대한.pdf): 정렬 알고리즘 분석 및 실험 결과를 포함합니다.
2. [Benchmark.kt](./assets/benchmark.kt): 벤치마크의 핵심 소스 파일입니다.
